...
const ChallengeScroll = (props) => {
    ...
    function makeIssuedChallengeObj(input){
        return (<IssuedChallengeObj key = {input["challengeId"]} index = {input.exercise.exerciseName} >{input}</IssuedChallengeObj>);
    }

    function makeSentChallengeObj(input){
        return (<SentChallengeObj key = {input["challengeId"]} index = {input.exercise.exerciseName}>{input}</SentChallengeObj>);
    }

    function makeReceivedChallengeObj(input){
        return (<ReceivedChallengeObj key = {input["challengeId"]} index = {input.exercise.exerciseName}>{input}</ReceivedChallengeObj>);
    }

    function makeGlobalChallengeObj(input){
        return (<GlobalChallengeObj key = {input["challengeId"]} index = {input.exercise.exerciseName}>{input}</GlobalChallengeObj>);
    }
